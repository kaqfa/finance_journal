...
const financeAPI = {
  // Wallet endpoints
  getWallets: (params?: { page?: number; page_size?: number; search?: string; ordering?: string }) => 
    authApi.get<PaginatedResponse<WalletList>>(createEndpoint(`${FINANCE_BASE}/wallets/`), { params }),
  ...
  recalculateWallet: (id: number) =>
    authApi.post<Wallet>(createEndpoint(`${FINANCE_BASE}/wallets/${id}/recalculate/`)),

  // Transaction endpoints
  getTransactions: (params?: { wallet?: number; limit?: number; offset?: number }) => 
    authApi.get<{
      count: number;
      next: string | null;
  ...
  getTransaction: (id: number) =>
    authApi.get<Transaction>(createEndpoint(`${FINANCE_BASE}/transactions/${id}/`)),
  
  createTransaction: (data: CreateTransactionData) =>
    authApi.post<Transaction>(createEndpoint(`${FINANCE_BASE}/transactions/`), data),
  
  updateTransaction: (id: number, data: Partial<CreateTransactionData>) =>
    authApi.put<Transaction>(createEndpoint(`${FINANCE_BASE}/transactions/${id}/`), data),
  ...
  createCategory: (data: { name: string; type: 'income' | 'expense'; icon?: string; color?: string }) =>
    authApi.post<Category>(createEndpoint(`${FINANCE_BASE}/categories/`), data),

  // Tag endpoints
  getTags: () =>
    authApi.get<Tag[]>(createEndpoint(`${FINANCE_BASE}/tags/`)),
